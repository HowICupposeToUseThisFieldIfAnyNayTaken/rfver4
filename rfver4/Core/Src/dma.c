...
void dma_rx_reinit(uint8_t uart_num,uint16_t rx_volume, uint8_t *rx_pointer){
	switch (uart_num){
	case UART1_NUMBER:
...
void dma_tx_reinit(uint8_t uart_num,uint16_t tx_volume, uint8_t *tx_pointer){
	switch (uart_num){
	case UART1_NUMBER:
...
int16_t dma_init(uint8_t uart_num, uint16_t tx_volume, \
	...
	case UART1_NUMBER://**********uart1**********//
		...
		UART1_RX_CHANEL->CPAR = &(USART1->DR);//адрес перифирии UART1
		UART1_RX_CHANEL->CMAR = rx_pointer;//вносим адрес массива куда писать значения
		UART1_RX_CHANEL->CNDTR = rx_volume;//количество передач
		...
		UART1_TX_CHANEL->CPAR = &(USART1->DR);//адрес перифирии UART1
		UART1_TX_CHANEL->CMAR = tx_pointer;//вносим адрес массива откуда брать значения
		UART1_TX_CHANEL->CNDTR = tx_volume;//количество передач
	...
	case UART2_NUMBER://**********uart2**********//
		...
		UART2_RX_CHANEL->CPAR = &(USART2->DR);//адрес перифирии UART1
		UART2_RX_CHANEL->CMAR = rx_pointer;//вносим адрес массива куда писать значения
		UART2_RX_CHANEL->CNDTR = rx_volume;//количество передач
		//из перифирии 8бит в память 8бит низкий приоритет
		//адрес памяти инкрементируется, адрес перифирии нет, не цикличный мод
		...
		UART2_TX_CHANEL->CPAR = &(USART2->DR);//адрес перифирии UART1
		UART2_TX_CHANEL->CMAR = tx_pointer;//вносим адрес массива откуда брать значения
		UART2_TX_CHANEL->CNDTR = tx_volume;//количество передач
		//из памяти 8бит в перифирию 8бит низкий приоритет
		//адрес памяти инкрементируется, адрес перифирии нет, не цикличный мод
